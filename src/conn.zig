...
test "Conn: auth trust (no pass)" {
    var conn = try Conn.open(t.io, t.allocator, t.connectOpts());
    defer conn.deinit();
...
test "PG: eager error conn state" {
    var pool = try lib.Pool.init(t.io, t.allocator, .{ .size = 1, .auth = t.authOpts(.{}), .connect = t.connectOpts() });
    defer pool.deinit();
...
test "open URI" {
    const uri_str = try std.fmt.allocPrint(
        t.allocator,
        "postgresql://postgres:postgres@{s}:{d}/postgres?tcp_user_timeout=5000",
        .{ t.envHost(), t.envPort() },
    );
    defer t.allocator.free(uri_str);
...
test "Conn: TLS required" {
    {
        var conn = try Conn.open(t.io, t.allocator, .{ .tls = .off, .host = t.envHost(), .port = t.envPort() });
        defer conn.deinit();
...
test "Conn: TLS verify-full" {
    try t.expectError(error.SSLCertificationVerificationError, Conn.open(t.io, t.allocator, .{ .tls = .{ .verify_full = null }, .host = t.envHost(), .port = t.envPort() }));
